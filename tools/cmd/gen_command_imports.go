...
var registerCommandTemplate = template.Must(template.New("register_commands").Parse(`// Code generated by the Hanamaru command linker; DO NOT EDIT.
package main
	...
	err := filepath.Walk("commands", func(path string, info os.FileInfo, err error) error {
		...
		if file != nil {
			if len(file.Comments) > 0 {
				spew.Dump(file.Comments)
				if strings.Contains(file.Comments[0].List[0].Text, "go:build") {
					return nil
				}
			}
